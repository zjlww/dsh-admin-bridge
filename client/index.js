    ...
    function PermissionControl({ sessionId, value, locked, command }) {
      ...
      return h(React.Fragment, null,
        ...
        h('dialog', { ref: dialog, 'aria-label': 'Sudo access',
          ...
          h('p', null, 'This grants Full access to the filesystem: the agent can modify or delete files outside the workspace and run sensitive external commands with fewer confirmations. It also authorizes only the exact administrator commands below. Changes cannot automatically be undone. Continue only if you trust this task.'),
          ...
          commands.length ? h('ul', null, ...commands.map(op => h('li', { key: op.id, style: { marginBottom: 12 } },
            ...
            h('small', null, 'Command timeout: ', op.timeoutSeconds, ' seconds')))) : h('p', null, 'No administrator operations are configured.'),
